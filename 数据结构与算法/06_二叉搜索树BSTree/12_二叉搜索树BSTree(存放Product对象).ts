// import Node from "../IList/Node.1";
import { btPrint, PrintableNode } from "hy-algokit"; // Binary Tree Print 打印二叉树
...
interface IComparatot<T> {
  (c1: T, c2: T): number;
}

class BSTree<T> {
  private root: TreeNode<T> | null = null;
  // private comparator: IComparatot<T> | null = null;
  // constructor(comparator: IComparatot<T>) {
...
class Product {
  ...
  // 仅仅用于对象比较 不能自定义指定打印
  // 想要自定义打印 取决于 打印的框架支不支持！！！！
  valueOf() {
    return this.price;
  }
}

const p1 = new Product("iPhone", 1999);
const p2 = new Product("huawei", 2999);
const p3 = new Product("xiaomi", 5999);
const p4 = new Product("oppo", 1699);
const p5 = new Product("vivo", 1200);
